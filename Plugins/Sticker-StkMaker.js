...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      let out
      try {
        stiker = await sticker(img, false, global.bname, global.oname)
      } catch (e) {
        console.error(e)
      } finally {
        if (!stiker) {
          ...
          stiker = await sticker(false, out, global.bname, global.oname)
        }
      }
    } else if (args[0]) {
      if (isUrl(args[0])) stiker = await sticker(false, args[0], global.bname, global.oname)
